...
testParser p s = either (\ (ErrParse _ _ e) -> error (show e)) id (parseWithError p "<test>" s)
...
holeP       = reserved "_" >> spaces >> return (RHole $ uTop $ Reft ("VV", [hole])) <?> "type hole"
holeRefP    = reserved "_" >> spaces >> return (RHole . uTop) <?> "type hole"
...
bareAll
  = do reserved "forall"
       try bareAllExprP <|> try bareAllS <|> bareAllP

bareAllExprP
  = do zs <- brackets $ sepBy1 exBindP comma 
       dot
       t  <- bareTypeP
...
measureP 
  = do (x, ty) <- tyBindP  
       whiteSpace
       eqns    <- grabs $ measureDefP (val x) $ (rawBodyP <|> tyBodyP ty)
...
classP
  ...
  where
    mb Nothing   = []
    mb (Just xs) = xs
    superP = maybeP (parens ( liftM (toRCls <$>)  (bareTypeP `sepBy1` comma)) <* reservedOp "=>")
    toRCls x = x
